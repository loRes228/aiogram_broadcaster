...
class StorageKey(NamedTuple):
    ...
    @classmethod
    def build(cls, prefix: str, mailer_id: int) -> "StorageKey":
        return StorageKey(
            chats=f"{prefix}:{mailer_id}:chats",
            settings=f"{prefix}:{mailer_id}:settings",
        )
...
class Storage:
    ...
    async def set_data(self, mailer_id: int, data: MailerData) -> None:
        key = StorageKey.build(prefix=self.key_prefix, mailer_id=mailer_id)
